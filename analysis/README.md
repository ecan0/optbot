...
3. Bind private, snapshot-specific Athena external tables to a final snapshot location with the authorized snapshot-maintenance role. Record only safe table aliases in `source_binding`; set `immutable_location_verified` to `true` only after verifying both aliases are not the moving current tables and the required retention/immutability control is in place. A timestamped prefix alone is not evidence of WORM retention. Do not use moving `responses_quantitative` or `response_quality` as the analysis source.
